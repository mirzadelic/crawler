...
class KupujemprodajemSpider(BaseSpider):
    ...
    def parse(self, response):
        ads = response.css(
            '#adListContainer div.item:not(.head)'
        ).getall()
    ...
    def get_next_url(self, response):
        next_page = response.xpath(
            u"//div[contains(@class, 'pageBarHolder')][1]/ul[contains(@class, 'pagesList')]/li[contains(@class, 'this-page')]/following-sibling::li[1]/a/text()").extract()
